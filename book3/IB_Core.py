"""
IB_Core - the machine, with the two openings Book Three needs.

Book Two's copy of this file is where Chapter 8 left the evaluator, and Book
Two never opened it again.  That import,

    from IB_Core import lEval, global_env, lisp_str

was the whole argument of Book Two written as one line: everything built from
there runs *in front of* the machine, and a backend is something you use
without opening.

Book Three is the ring outside that one, and an outer ring may reach in.  It
opens this file in exactly two places, and then leaves it alone:
...
the machine runs exactly as it did.
...
class MachineError(Exception):
  """An error with the machine's registers attached.
    ...
    reached the reader was a traceback running down into this file.  Chapter
    19 wants C, E and K as they stood at the failure, so the machine catches,
    attaches them, and re-raises.
  """
  def __init__(self, message, C, E, K):
...
# A tool sets this to a function of (C, E, K); the machine calls it once for
# every expression it is about to evaluate.  None means the machine is exactly
# the one Book Two finished.  Set it on the MODULE, not on a name imported
# from it: `IB_Core.step_hook = f`.
step_hook = None
...
def lEval(expr, env):
  ...
  try:
    while True:
      ...
      # ----- Begin state EVAL -----
      while True:
        if step_hook is not None:        # Book Three, opening one of two
          step_hook(C, E, K)
